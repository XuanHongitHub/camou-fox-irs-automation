...
clean_csv = Path(r"F:\Herd\fox-auto\outputs\drive_cleaned_v2\26-09-2026_-_1271_DONE\26-09-2026 - 1271 DONE_CLEANED.csv")
orig_csv = Path(r"F:\Herd\fox-auto\outputs\drive_cleaned_v2\26-09-2026_-_1271_DONE\26-09-2026 - 1271 DONE_ORIGINAL.csv")

df_clean = pd.read_csv(clean_csv, dtype=str).fillna('')
df_orig = pd.read_csv(orig_csv, dtype=str).fillna('')

# Find rows where original == clean (i.e. was NOT changed)
unchanged_rows = []
...
print(f"Tổng số records KHÔNG ĐỔI trong Lô 1271: {len(unchanged_rows)}")

# Take first 50 unchanged rows and send to Census Geocoder
...
print("Gửi 50 record 'không đổi' lên US Census Geocoder để kiểm tra thực tế...")
...
print(f"\nKết quả kiểm tra 50 mẫu 'không đổi': {matches} khớp Census, trong đó CÓ {wrong_count} CÁI BỊ SAI MÃ ZIP!")
